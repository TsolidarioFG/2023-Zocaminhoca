...
import { readPedidos, savePedidos } from "../../backend/OrderService";
...
const OrdersList = ({pedidosFilename, setSuccessAlert, setErrorAlert, jobId}) => {
    ...
    const handleSaveOrders = () => {
        savePedidos(orders, ()=>{
                                    setSuccessAlert("Pedidos gardados correctamente");
                                    markJobAsDone(jobId, ()=>{},()=>{});
                                    setOrders([]);
    ...
    useEffect(() => {
        readPedidos(pedidosFilename, (response)=>{setOrders(response)}, ()=>{});
    ...
    return (
        <div>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {showOrders.map((row, index) => (
                                        <TableRow className={`${tableStyles.selectableRow} ${index===selectedOrderIdx ? tableStyles.selectedRow : ''}`} key={row.webId || index} onClick={() => handleOrderSelection(row,index)}>
            ...
            <div className={styles.saveButtonContainer}> 
                <button onClick={handleSaveOrders} className={styles.saveButton}>Gardar</button>
            </div>
        </div>
    )
}
